...
// 在文件顶部添加声明
extern "C" void setGpioHigh() __attribute__((naked));

// 在文件中添加汇编函数
void setGpioHigh() {
    asm volatile (
        // 设置PIOB的基地址 (0x400E1000)
        "ldr r0, =0x400E1000\n\t"    // PIOB外设基地址
        "mov r1, #3\n\t"             // 准备位掩码
        "lsl r1, r1, #21\n\t"        // 左移21位 (对应Arduino引脚13, PB21)
        "str r1, [r0, #0x30]\n\t"    // 写入SODR寄存器(Set Output Data Register)
        "bx lr\n\t"                  // 返回
    );
}
// void setGpioHigh() {
//     asm volatile (
//         // 加载PIOB基地址 (0x400E1000)
//         "ldr r0, =0x400E1000\n\t"    // PIOB外设基地址
        
//         // 直接设置位掩码，同时包含PB21和PB22
//         "mov r1, #(3 << 21)\n\t"     // 将1左移21位后的值再乘2加1，等于11 << 21
...
//     // // 记录开始时间
//     // uint32_t startCycles = getDwtCyccnt();
...
//     // // 记录结束时间
//     // uint32_t endCycles = getDwtCyccnt();
...
#ifdef MBTCP
__attribute__((section(".custom_section"))) void handle_tcp()
{
    #ifdef MBTCP_ETHERNET
...
#endif
// int mb_buffer_overflow = 0;
// static int ifExecutionCount = 0; // 添加静态计数器
#ifdef MBSERIAL
void handle_serial()
{
    mb_frame_len = 0;
    ...
    {
        // ifExecutionCount++; // 增加计数
        // Serial.print("if语句执行次数: ");
        // Serial.println(ifExecutionCount);
        // int byteCount = 0;  // 添加计数器
        // while ((*mb_serialport).available())
        // {
        //     uint8_t data = (*mb_serialport).read();
        ...
        // // 在原有代码处添加标记设置
        // if((*mb_serialport).available() > MAX_MB_FRAME) {
        //     mb_buffer_overflow = 1;  // 标记缓冲区溢出
        //     // ...existing code...
        // }

        // if((*mb_serialport).available() < 6) {
        //     mb_buffer_overflow = 2;  // 标记缓冲区溢出
        //     // ...existing code...
        // }
